"""
This code is modified based on https://github.com/Sense-X/UniFormer
"""
import argparse
import time
import yaml
...
parser.add_argument('--dataset', '-d', metavar='NAME', default='WDF',
...
parser.add_argument('--warmup-epochs', type=int, default=5, metavar='N',
                    help='epochs to warmup LR, if scheduler supports')  # original = 5
...
parser.add_argument('--token-label-data', type=str, default='',
                    help='path to token_label data')
parser.add_argument('--token-label-size', type=int, default=2, metavar='N',
                    help='size of result token label map')  # default=1
...
# Finetune
parser.add_argument('--finetune', default='/code/SSAAFormer/token_labeling/exp/uniformer_large_ls_tl_224/uniformer_large_ls_tl_224.pth', type=str, metavar='PATH',
                    help='path to checkpoint file (default: none)')     # 1/

parser.add_argument('--csv-res', default='', type=str)

parser.add_argument('--train-csv', default='', type=str)
parser.add_argument('--test-csv', default='', type=str)
...
def main():
    ...
    if args.token_label_data:
        ...
        dataset_train = FakeFaceDataset(csv_file=args.train_csv)     # 2/
    dataset_eval = FakeFaceDataset(csv_file=args.test_csv)       # 3/
    ...
    try:
        for epoch in range(start_epoch, 3):
            ...
            eval_metrics = validate('val_', epoch, model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)

            if model_ema is not None and not args.model_ema_force_cpu:
                if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
                    distribute_bn(model_ema, args.world_size, args.dist_bn == 'reduce')
                ema_eval_metrics = validate(
                    'ema_', epoch, model_ema.module, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, log_suffix=' (EMA)')
...
def train_one_epoch(
    ...
    print("Train_one_epoch : ", output_dir, "; ", saver)
...
def validate(pre, epoch, model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            predict_write.extend(predicted.tolist())
            label_write.extend(target.tolist())
        ...
        df = pd.DataFrame({'predict': predict_write, 'label': label_write})
        df.to_csv(os.path.join(args.csv_res, pre + str(epoch)+'.csv'), sep=',', index=None)

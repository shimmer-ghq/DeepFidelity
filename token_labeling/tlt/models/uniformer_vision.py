...
class CBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.attn_f = nn.Conv2d(dim, dim, 5, padding=2, groups=dim)
        # self.w = nn.Parameter(torch.ones(2))    # for flip feature map fusion
    ...
    def forward(self, x):
        ...
        # for feature map vision
        if self.ls:
            feature_ret = self.conv1(self.norm1(x))
            x = x + self.drop_path(self.gamma_1 * self.conv2(self.attn(feature_ret)))
            x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.conv2(self.attn(self.conv1(self.norm1(x)))))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        # return x, feature_ret

        return x

class CBlock_flip(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.pos_embed = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
    ...
    def forward(self, x):
        ...
        # exp4 / exp5
        if self.ls:
            attn_ori = self.attn(self.conv1(self.norm1(x)))
            attn_flip = torch.flip(attn_ori, dims=[3])
            feature_ret_ori = attn_ori
            feature_ret_flip = attn_flip

            # 归一化权重
            w1 = torch.exp(self.w[0]) / torch.sum(torch.exp(self.w))
            w2 = torch.exp(self.w[1]) / torch.sum(torch.exp(self.w))

            attn_tmp = attn_ori * w1 + attn_flip * w2
            feature_ret_add = attn_tmp
            x = x + self.drop_path(self.gamma_1 * self.conv2(attn_tmp))
            x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        else:
            attn_ori = self.attn(self.conv1(self.norm1(x)))
            attn_flip = torch.flip(attn_ori, dims=[3])
            ...
            attn_tmp = attn_ori * w1 + attn_flip * w2
            feature_ret_add = attn_tmp
            x = x + self.drop_path(self.conv2(attn_tmp))
            x = x + self.drop_path(self.mlp(self.norm2(x)))

        return x, feature_ret_ori, feature_ret_flip, feature_ret_add
...
class UniFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks1:
            x, fea_ori, fea_flip, fea_add = blk(x)
            fea_visions_ori.append(fea_ori)
            fea_visions_flip.append(fea_flip)
            fea_visions_add.append(fea_add)
        ...
        if self.return_dense:
            ...
            if not self.training:
                # print("return x_cls + 0.5 * x_aux.max(1)[0]")
                # for feature map vision
                return x_cls + 0.5 * x_aux.max(1)[0], fea_visions_ori, fea_visions_flip, fea_visions_add
            ...
            if self.mix_token and self.training:
                ...
                x_aux = temp_x
                x_aux = x_aux.reshape(x_aux.shape[0], patch_h * patch_w, x_aux.shape[-1])
            # print("return x_cls, x_aux, (bbx1, bby1, bbx2, bby2)")
            return x_cls, x_aux, (bbx1, bby1, bbx2, bby2)
        # print("return x_cls, fea_visions1, fea_visions2")
        # for feature map vision
        return x_cls, fea_visions_ori, fea_visions_flip, fea_visions_add
        # return x_cls

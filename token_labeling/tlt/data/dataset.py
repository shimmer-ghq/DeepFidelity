...
def find_images_and_targets(folder, types=IMG_EXTENSIONS, class_to_idx=None, leaf_name_only=True, sort=True):
    df = pd.read_csv(r'/data/UniFormer-wyk/UniFormer0/image_classification/dataset/wdf/train.csv')
    filenames = df['img'].tolist()
...
class DatasetTokenLabel(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        score_path = os.path.join(
            self.label_root, ((path.split('/')[-3]).split('_')[0]), path.split('/')[-2], ((path.split('/')[-1]).split('.')[0]) + '.pt') # label_root = .../out
